...
def get_model(model_type, 
            ...
            tanh_exploration) : 
    if model_type == "LSTM" : 
        model = solver_LSTM(membedding_size,
            hidden_size,
            seq_len,
            n_glimpses,
            tanh_exploration)
    elif model_type == "Attention" : 
        model = solver_Attention(membedding_size,
            hidden_size,
            seq_len,
            n_glimpses,
            tanh_exploration)    
...
def compute_distance(nodes, order) :
    dist = 0
    for i in range(len(order)-1):
        dist += tsp_heuristic.calc_dist(nodes[order[i]], nodes[order[i+1]])
    return dist
...
def graph_visualize(nodes, order, graph_label='graph') :
        ...
        G = nx.DiGraph()
        distance = compute_distance(nodes, order)
        ...
        plt.savefig(graph_label + '_tour' + '.png')  
        plt.show()
...
def TSP_RL(args) : 
    ...
    #train_dataset = tsp_generator.TSPDataset(args.seq_len, 1)
    model = get_model(
            args.model_type,
            args.embedding_size,
            args.hidden_size,
    ...
    reward, _, _= batch_test(test_dataset, model)
    ...
    reward, _, _ = batch_test(test_dataset, model) #reward is the tour distance
...
class HPOptimizer :
    ...
    def define_model(self, trial) : 
        ...
        model = get_model(self.args.model_type, embedding_size, hidden_size, self.args.seq_len, 2, 10)
...
class ModelVsHeuristics :
    ...
    def compare(self, model, dataset=None, print_graph=False) : 
        ...
        if print_graph : 
            ...
            for i, pointset in tqdm(dataset):
                graph_visualize(pointset, [], graph_label='Only cities')
                #print(pointset)
